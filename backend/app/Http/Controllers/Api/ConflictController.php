        ...
        try {
            ...
            try {
                if ($conflict && isset($conflict->project_id)) {
                    ...
                    $this->llmService->incrementalKBUpdate($conflict->project_id, [$resolvedDoc]);
                }
            } catch (\Exception $e) {
...
class ConflictController extends Controller
{
    ...
    public function resolveConflict(Request $request, int $conflictId)
    {
        ...
        try {
            ...
            // Append a small resolved marker to KB so searches can reflect resolution state
            try {
                if ($conflict && isset($conflict->project_id)) {
                    $resolvedDoc = [
                        'content' => "Conflict resolved: {$conflict->id} - {$request->input('resolution_notes')}",
    ...
    public function deleteConflict(int $conflictId)
    {
        try {
            ...
            try {
                ...
                // Log but do not fail the API delete - allow DB to be authoritative
                Log::warning('Failed to remove conflict from KB after delete', [
                    'conflict_id' => $conflictId,
                    'project_id' => $projectId,
                    'error' => $e->getMessage()
